...
# --- ドロップダウン用のリスト定義（Excelファイルから読み込み） ---

# 職員名リストの読み込み
try:
    df_staff = pd.read_excel("職員名.xlsx")
    staff_names = df_staff["職員名"].tolist() if "職員名" in df_staff.columns else []
except FileNotFoundError:
    staff_names = ["職員名ファイルが見つかりません"]
    st.warning("職員名ファイル '職員名.xlsx' が見つかりませんでした。デフォルト名を使用します。")

# 男性利用者名リストの読み込み
try:
    df_male_users = pd.read_excel("男性利用者.xlsx")
    male_users = df_male_users["氏名"].tolist() if "氏名" in df_male_users.columns else []
except FileNotFoundError:
    male_users = []
    st.warning("男性利用者ファイル '男性利用者.xlsx' が見つかりませんでした。")

# 女性利用者名リストの読み込み
try:
    df_female_users = pd.read_excel("女性利用者.xlsx")
    female_users = df_female_users["氏名"].tolist() if "氏名" in df_female_users.columns else []
except FileNotFoundError:
    female_users = []
    st.warning("女性利用者ファイル '女性利用者.xlsx' が見つかりませんでした。")

all_users = sorted(male_users + female_users) # 男女合わせた利用者リスト
...
if menu == "日報（全体・プロジェクト）":
    ...
    with st.form("daily_form"):
        ...
        # 本日の主役を利用者リストから選択
        star_user = st.selectbox("本日の「主役」（特に輝いていた利用者様）", ["選択してください"] + all_users)
...
# --- モード3：データ確認・ダウンロード ---
    ...
    if os.path.exists(INDIVIDUAL_LOG_FILE):
        ...
        st.info("個別記録のデータはまだありません。")
''')
print("Streamlitアプリのコードが 'app.py' として保存されました。")